...
namespace CSharpSuction.Generators.TypeScript
{
    ...
    public class EmitTypeScript : Emit
    {
        ...
        private void EmitCode(SyntaxNode value)
        {
            ...
            else if (value is SwitchStatementSyntax)
            {
                EmitSwitchStatement((SwitchStatementSyntax)value);
            }
            else if (value is CaseSwitchLabelSyntax)
            {
                EmitCaseSwitchLabel((CaseSwitchLabelSyntax)value);
            }
            else if (value is DefaultSwitchLabelSyntax)
            {
                EmitDefaultSwitchLabel((DefaultSwitchLabelSyntax)value);
            }
            else
            {
        ...
        private void EmitDefaultSwitchLabel(DefaultSwitchLabelSyntax value)
        {
            WriteLine("default:");
        }

        private void EmitCaseSwitchLabel(CaseSwitchLabelSyntax value)
        {
            WriteLine("case " + MakeCode(value.Value) + ":");
        }

        private void EmitSwitchStatement(SwitchStatementSyntax value)
        {
            WriteLine("switch(" + MakeCode(value.Expression) + ")");
            EnterBlock();

            foreach (var e in value.Sections)
            {
        ...
        private string MakeCode(SyntaxNode value)
        {
            ...
            {
                throw new NotImplementedException("unsupported syntax [" + value.GetType().Name + "]: " + value);
            }
        }
        ...
        private string MakeLiteralExpression(LiteralExpressionSyntax value)
        {
            ...
            {
                // convert
                return ConvertToSimpleLiteral(s.Substring(1));
            }
            else
            {
                return s;
        ...
        private string ConvertToSimpleLiteral(string s)
        {
